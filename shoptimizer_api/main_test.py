...
class MainTest(parameterized.TestCase):
  ...
  def test_entries_missing_in_request_returns_error(self):
    ...
    response = self.test_client.post(
        f'{main._V1_BASE_URL}/batch/optimize', json=request_body_missing_entries
    )
    response_data = response.data.decode('utf-8')
    response_dict = json.loads(response_data)

    self.assertEqual(
        'Request must contain entries as a key.', response_dict['error-msg']
    )
    self.assertEqual(http.HTTPStatus.BAD_REQUEST, response.status_code)

  def test_entries_with_no_list_of_products_returns_error(self):
    request_body_missing_entries_product_list = (
        requests_bodies.INVALID_ENTRIES_MISSING_PRODUCT_LIST
    )
    response = self.test_client.post(
    ...
    self.assertEqual(
        'Entries must contain a list of products.', response_dict['error-msg']
    )
    self.assertEqual(http.HTTPStatus.BAD_REQUEST, response.status_code)

  @parameterized.named_parameters([
      {'testcase_name': 'title_is_numeric', 'attribute': 'title'},
      {'testcase_name': 'description_is_numeric', 'attribute': 'description'},
  ])
  def test_numeric_attribute_is_converted_to_string(self, attribute):
    ...
    self.assertIsInstance(
        response_dict['optimized-data']['entries'][0]['product'][attribute], str
    )
  ...
  def test_invalid_query_string_param_returns_bad_request(self):
    ...
    self.assertEqual(
        (
            ...
            'Shoptimizer API query string parameter.'
        ),
  ...
  def test_optimizer_config_override_overwrites_default_optimizer(self):
    ...
    """
    override_config_contents_base64 = base64.b64encode(
        override_config_contents_raw
    )
  ...
  def test_plugin_not_run_when_parameter_set_to_false(self):
    ...
    response = self.test_client.post(
        f'{main._V1_BASE_URL}/batch/optimize?my-plugin=false', json=request_body
    )
    response_data = response.data.decode('utf-8')
    response_dict = json.loads(response_data)
    optimization_results = response_dict['plugin-results']
