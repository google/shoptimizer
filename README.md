...
call (for the V1 endpoint) or the Merchant Center API (for the V2 endpoint).
Shoptimizer sanitizes and optimizes the product data, and returns it to
the client. This optimized product data can then be sent to Content API for
Shopping / Merchant Center API.
...
[Merchant Center API](https://developers.google.com/merchant/api)           | The modernized programmatic interface for interacting with Merchant Center.
Content / Merchant API Client                                               | A solution for sending product data to Content API for Shopping or Merchant Center API. This is a generic term and does not refer to a specific codebase. For example, this could be a Python program that uses Google APIs to make calls to upload data to Merchant Center.
Shoptimizer                                                                 | A REST API that can be integrated into a Content / Merchant API Client to automate improvements to product data before it is sent to Merchant Center.
Optimizer                                                                   | An isolated block of optimization logic within Shoptimizer. Individual optimizers can be enabled and disabled.
Built-in Optimizer                                                           | An optimizer that comes bundled with Shoptimizer.
...
----------------------------- | --------------------------- | ------------------------------------------ | -----------
adult-optimizer               | _Optimization_              | adult                                      | Overwrites the "adult" attribute on a product when the optimizer determines that the product is adult-oriented.
...
free-shipping-optimizer       | _Sanitization_              | shipping                                   | Adds a shipping object with a value of "0" to the "shipping" attribute when a free shipping pattern is detected in the title. This will lead to the product avoiding disapproval. It must be run before title-optimizer.
identifier-exists-optimizer   | _Sanitization_              | identifierExists                           | Removes invalid identifierExists fields. Items that have a brand, mpn, or gtin set and identifierExists as "false" cause disapproval, so this optimizer will delete the identifierExists value in these cases, which defaults the value to true via the API. This will lead to the product avoiding disapproval.
image-link-optimizer          | _Sanitization/Optimization_ | imageLink, additionalImageLink             | Identifies imageLinks that are likely to cause this product to be disapproved (invalid URL format, file size is too large, included ML model determines that the image contains an overlay), and if at least one additionalImageLink is not likely to be disapproved, swaps the imageLink URL with the best available additionalImageLink. See the documentation for [Image Link Optimizer](./docs/image-link-optimizer.md) for details.
...
shopping-exclusion-optimizer  | _Sanitization_              | excludedDestinations, includedDestinations | Detects products that should not be meant to be served as Shopping ads (these types of products may cause disapproval in Merchant Center) using term matches from a customizable config file against the product's title, and upon detection of these products, the optimizer will add the "Shopping Ads" destination to the API request attribute "excludedDestinations" and remove the Shopping Ads destination from the API request attribute "includedDestinations", if it exists.
size-length-optimizer         | _Optimization_              | size (or sizes)                            | Trims the size attribute to a maximum of 100 characters. For V1 payloads (which use a list of sizes), it also ensures that the list contains only a single value. This prevents unnecessarily bloated sizes from being rejected or rendering poorly.
